...
export const RoadmapCard: React.FC<RoadmapCardProps> = ({ index, lastIndex, data, isNextItemIncomplete }) => {
    return (
        ...
        >
            <div className="outlined-card w-full relative md:w-[calc(50%-40px)] xl:w-[calc(50%-48px)] 3xl:w-[calc(50%-64px)]">
                <p className="light-gray-rounded-card-heading !font-medium text-primary 3xl:!font-semibold">
                ...
                {/* Dot */}
                <div
                    className={`roadmap-dot top-0 -left-6 flex justify-center items-center ${data.isCompleted ? 'bg-primary text-white font-bold' : 'bg-white'} ${(index + 1) % 2 !== 0 ? 'md:-left-[52px] xl:-left-[63px] 3xl:-left-[84px]' : 'md:left-[calc(100%+30px)] xl:left-[calc(100%+35px)] 3xl:left-[calc(100%+47px)]'}`}
                >
                    {data.isCompleted && <BiCheck className="md:text-xl xl:text-3xl 3xl:text-4xl" />}
                </div>
            </div>
            ...
            {data.isCompleted && isNextItemIncomplete(index) && (
                <div className="roadmap-dot !top-1/2 !-left-[23px] bg-primary md:!left-[calc(50%-11px)] xl:!left-[calc(50%-14px)] 3xl:!left-[calc(50%-18px)]">
            ...
            {data.isCompleted && isNextItemIncomplete(index) && (
                <div
                    className={`hidden bg-black text-white absolute top-[45%] rounded py-2 px-4 md:block xl:py-3 xl:px-6 ${(index + 1) % 2 !== 0 ? 'left-[calc(50%-148px)] xl:left-[calc(50%-188px)] 3xl:left-[calc(50%-200px)]' : 'right-[calc(50%-148px)] xl:right-[calc(50%-188px)] 3xl:right-[calc(50%-200px)]'}`}
                >
